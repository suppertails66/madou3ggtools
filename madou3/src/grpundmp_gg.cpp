...
int main(int argc, char* argv[]) {
  if (argc < 4) {
    ...
    cout << "  r    Specify patterns per row (default: 16)" << endl;
    return 0;
  }
...
//    TIfstream palifs(argv[4], ios_base::binary);
  ...
  char* patternsPerRowName = TOpt::getOpt(argc, argv, "-r");
  if (patternsPerRowName != NULL) {
    patternsPerRow
      = TStringConversion::stringToInt(std::string(patternsPerRowName));
  }
