...
const static int region_locTable_addr = 0x2766;
...
void dumpString(TStream& ifs, std::ostream& ofs, const TThingyTable& table,
  ...
  oss << "// ";
  bool copiedContentPending = false;
  while (true) {
    ...
    if ((result.id == op_dict)) {
      if (copiedContentPending) {
        copiedContentPending = false;
        oss << endl;
        oss << "// ";
      }
    ...
    if ((result.id == op_leftbox)
        ...
        || ((result.id >= 0xC5) && (result.id <= 0xD1)
            && (result.id != 0xC7))
        ) {
//      oss << endl;
      if (!copiedContentPending) {
        oss << endl << endl;
        copiedContentPending = true;
      }
      
      oss << resultStr;
      
      if (opHasParam(result.id)) {
...
//  int baseBank = getRegionBaseBank(regionNum);
  ifs.seek(region_locTable_addr + (regionNum * 3));
...
int main(int argc, char* argv[]) {
  if (argc < 3) {
    cout << "Madou Monogatari III (Game Gear) script dumper" << endl;
...
//  string tableName = string(argv[2]);
  ...
  tablestd.readSjis(string("table/madou3.tbl"));
