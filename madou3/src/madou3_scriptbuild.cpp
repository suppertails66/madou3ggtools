...
string getStringName(Madou3ScriptReader::ResultString result) {
...
void exportRawResults(Madou3ScriptReader::ResultCollection& results,
                      std::string filename) {
...
void exportRawResults(TStream& ifs,
                      std::string filename) {
  Madou3ScriptReader::ResultCollection results;
  Madou3ScriptReader(ifs, results, table)();
...
//    ofs.save((outPrefix + "script.bin").c_str());
  ...
  std::map<int, Madou3ScriptReader::ResultCollection>
    mappedStringBuckets;
  for (unsigned int i = 0; i < results.size(); i++) {
  ...
  for (std::map<int, Madou3ScriptReader::ResultCollection>::iterator it
         = mappedStringBuckets.begin();
       it != mappedStringBuckets.end();
       ++it) {
    int key = it->first;
    Madou3ScriptReader::ResultCollection& results = it->second;
    ...
    for (unsigned int i = 0; i < results.size(); i++) {
      Madou3ScriptReader::ResultString result = results[i];
  ...
  for (int i = 0; i < hashMask; i++) {
    std::map<int, Madou3ScriptReader::ResultCollection>::iterator findIt
...
int main(int argc, char* argv[]) {
  if (argc < 4) {
    cout << "Madou Monogatari III (Game Gear) script builder" << endl;
  ...
  {
    // read size table
    Madou3LineWrapper::CharSizeTable sizeTable;
    {
      TBufStream ifs;
...
//      ifs.open(("out/script/dialogue_all.txt"));
      ...
      Madou3LineWrapper(ifs, results, table, sizeTable)();
