...
LastBibleScriptReader::LastBibleScriptReader(
...
bool LastBibleScriptReader::operator()() {
  while (!src.eof()) {
    std::string line;
    src.getLine(line);
    ++lineNum;
...
void LastBibleScriptReader::loadThingy(const BlackT::TThingyTable& thingy__) {
  thingy = thingy__;
}
  
void LastBibleScriptReader::outputNextSymbol(TStream& ifs) {
  // literal value
  if ((ifs.remaining() >= 5)
      && (ifs.peek() == '<')) {
    int pos = ifs.tell();
...
//    std::cerr << std::dec << lineNum << " " << std::hex << result.id << " " << result.size << std::endl;
  ...
  throw TGenericException(T_SRCANDLINE,
                          "LastBibleScriptReader::outputNextSymbol()",
                          "Line "
...
void LastBibleScriptReader::flushActiveScript() {
  // write terminator
...
void LastBibleScriptReader::resetScriptBuffer() {
...
/*bool LastBibleScriptReader::checkSymbol(BlackT::TStream& ifs, std::string& symbol) {
...
void LastBibleScriptReader::processDirective(BlackT::TStream& ifs) {
  ...
  if (name.compare("LOADTABLE") == 0) {
    ...
    throw TGenericException(T_SRCANDLINE,
                            "LastBibleScriptReader::processDirective()",
                            "Line "
                              + TStringConversion::intToString(lineNum)
                              + ":\n  Unknown directive: "
                              + name);
...
void LastBibleScriptReader::processLoadTable(BlackT::TStream& ifs) {
  std::string tableName = TParse::matchString(ifs);
  TThingyTable table(tableName);
  loadThingy(table);
}

void LastBibleScriptReader::processStartMsg(BlackT::TStream& ifs) {
...
void LastBibleScriptReader::processEndMsg(BlackT::TStream& ifs) {
  flushActiveScript();
}

void LastBibleScriptReader::processIncBin(BlackT::TStream& ifs) {
...
void LastBibleScriptReader::processBreak(BlackT::TStream& ifs) {
  breakTriggered = true;
}
