...
BssmGgScriptReader::BssmGgScriptReader(
...
bool BssmGgScriptReader::operator()() {
  while (!src.eof()) {
    std::string line;
    src.getLine(line);
    ++lineNum;
...
void BssmGgScriptReader::loadThingy(const BlackT::TThingyTable& thingy__) {
  thingy = thingy__;
}
  
void BssmGgScriptReader::outputNextSymbol(TStream& ifs) {
  // literal value
  if ((ifs.remaining() >= 5)
      && (ifs.peek() == '<')) {
    int pos = ifs.tell();
...
//    std::cerr << std::dec << lineNum << " " << std::hex << result.id << " " << result.size << std::endl;
  ...
  throw TGenericException(T_SRCANDLINE,
                          "BssmGgScriptReader::outputNextSymbol()",
                          "Line "
...
void BssmGgScriptReader::flushActiveScript() {
  // write terminator
...
void BssmGgScriptReader::resetScriptBuffer() {
...
/*bool BssmGgScriptReader::checkSymbol(BlackT::TStream& ifs, std::string& symbol) {
...
void BssmGgScriptReader::processDirective(BlackT::TStream& ifs) {
  ...
  if (name.compare("LOADTABLE") == 0) {
    ...
    throw TGenericException(T_SRCANDLINE,
                            "BssmGgScriptReader::processDirective()",
                            "Line "
                              + TStringConversion::intToString(lineNum)
                              + ":\n  Unknown directive: "
                              + name);
...
void BssmGgScriptReader::processLoadTable(BlackT::TStream& ifs) {
  std::string tableName = TParse::matchString(ifs);
  TThingyTable table(tableName);
  loadThingy(table);
}

void BssmGgScriptReader::processStartMsg(BlackT::TStream& ifs) {
...
void BssmGgScriptReader::processEndMsg(BlackT::TStream& ifs) {
  flushActiveScript();
}

void BssmGgScriptReader::processIncBin(BlackT::TStream& ifs) {
...
void BssmGgScriptReader::processBreak(BlackT::TStream& ifs) {
  breakTriggered = true;
}
