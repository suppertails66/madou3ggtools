#include "madou3/Madou3ScriptReader.h"
...
Madou3ScriptReader::Madou3ScriptReader(
...
bool Madou3ScriptReader::operator()() {
  try {
    while (!src.eof()) {
      std::string line;
      src.getLine(line);
...
void Madou3ScriptReader::loadThingy(const BlackT::TThingyTable& thingy__) {
  thingy = thingy__;
}
  
void Madou3ScriptReader::outputNextSymbol(TStream& ifs) {
  // literal value
  if ((ifs.remaining() >= 5)
      && (ifs.peek() == '<')) {
    int pos = ifs.tell();
...
//    std::cerr << std::dec << lineNum << " " << std::hex << result.id << " " << result.size << std::endl;
  ...
  throw TGenericException(T_SRCANDLINE,
                          "Madou3ScriptReader::outputNextSymbol()",
                          "Line "
...
void Madou3ScriptReader::flushActiveScript() {
  // write terminator
...
void Madou3ScriptReader::resetScriptBuffer() {
...
/*bool Madou3ScriptReader::checkSymbol(BlackT::TStream& ifs, std::string& symbol) {
...
void Madou3ScriptReader::processDirective(BlackT::TStream& ifs) {
  ...
  if (name.compare("LOADTABLE") == 0) {
    ...
    throw TGenericException(T_SRCANDLINE,
                            "Madou3ScriptReader::processDirective()",
                            "Line "
                              + TStringConversion::intToString(lineNum)
                              + ":\n  Unknown directive: "
                              + name);
...
void Madou3ScriptReader::processLoadTable(BlackT::TStream& ifs) {
  std::string tableName = TParse::matchString(ifs);
  TThingyTable table(tableName);
  loadThingy(table);
}

void Madou3ScriptReader::processStartMsg(BlackT::TStream& ifs) {
...
void Madou3ScriptReader::processEndMsg(BlackT::TStream& ifs) {
  flushActiveScript();
}

void Madou3ScriptReader::processIncBin(BlackT::TStream& ifs) {
...
void Madou3ScriptReader::processBreak(BlackT::TStream& ifs) {
  breakTriggered = true;
}

void Madou3ScriptReader::processEndRegion(BlackT::TStream& ifs) {
  TParse::matchInt(ifs);
//  flushActiveScript();
  breakTriggered = true;
}
